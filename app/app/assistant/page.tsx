...
export default async function AssistantPage() {
  ...
  const [doctorQuery, settingsQuery, paymentQuery, typesQuery, intakeQuery, depositsQuery, knowledgeQuery, schedule, recurringUnavailability] = await Promise.all([
  ...
  return (
    <AssistantConfiguration doctorId={context.doctorId} doctor={doctorQuery.data} timezone={doctorQuery.data.timezone} schedule={schedule} recurringUnavailability={recurringUnavailability} settings={settingsQuery.data} payment={paymentQuery.data} appointmentTypes={typesQuery.data ?? []} intakeFields={intakeQuery.data ?? []} depositRules={depositsQuery.data ?? []} knowledgeItems={knowledgeQuery.data ?? []} />
  )
}

    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[1080px] text-left text-sm">
        <thead className="border-b border-zinc-200 bg-zinc-50 text-xs font-semibold uppercase tracking-wide text-zinc-500"><tr>
          <th className="px-4 py-3">Nombre</th><th className="px-4 py-3">Teléfono</th><th className="px-4 py-3">WhatsApp</th><th className="px-4 py-3">Estado</th><th className="px-4 py-3">Zona horaria</th><th className="px-4 py-3">Retell Agent ID</th><th className="px-4 py-3">Twilio number</th><th className="px-4 py-3">Google Calendar</th><th className="px-4 py-3">Acción</th>
        </tr></thead>
        <tbody className="divide-y divide-zinc-100">{doctors.map((doctor) => (
          <tr key={doctor.id} className="align-middle text-zinc-700">
            <td className="whitespace-nowrap px-4 py-4 font-medium text-zinc-950">{doctor.display_name}</td><td className="px-4 py-4">{valueOrDash(doctor.phone)}</td><td className="px-4 py-4">{valueOrDash(doctor.whatsapp)}</td><td className="px-4 py-4"><span className={doctor.status === 'ACTIVE' ? 'text-emerald-700' : 'text-zinc-500'}>{doctor.status === 'ACTIVE' ? 'Activo' : 'Inactivo'}</span></td><td className="px-4 py-4 font-mono text-xs">{doctor.timezone}</td><td className="px-4 py-4 font-mono text-xs">{valueOrDash(doctor.retell_agent_id)}</td><td className="px-4 py-4 font-mono text-xs">{valueOrDash(doctor.twilio_phone_number)}</td><td className="px-4 py-4 text-zinc-500">No conectado</td><td className="px-4 py-4"><div className="flex items-center gap-2"><a href={'/master/doctors/' + doctor.id + '/agenda'} className="rounded-md bg-zinc-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-zinc-700">Entrar</a><DoctorEditor doctor={doctor} /></div></td>
          </tr>
        ))}</tbody>
      </table>
    </div>

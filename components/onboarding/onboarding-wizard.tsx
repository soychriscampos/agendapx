...
import { deleteKnowledgeItem, saveAppointmentTypesForOnboarding, saveAssistantSettings, saveDepositRulesForOnboarding, saveIntakeFieldsForOnboarding, saveKnowledgeItem, savePaymentInstructions } from '@/lib/assistant/assistant-configuration'
...
type EditableKnowledgeItem = KnowledgeItem & { localKey: string }
type KnowledgeEditorState =
  | { mode: 'create'; item: EditableKnowledgeItem }
  | { mode: 'edit'; localKey: string; item: EditableKnowledgeItem }
  | null
...
export function OnboardingWizard({ doctorId, doctor, timezone, schedule, settings, payment, appointmentTypes, intakeFields, depositRules, knowledgeItems }: { doctorId: string; doctor: Doctor; timezone: string; schedule: DoctorSchedule; settings: Settings | null; payment: Payment | null; appointmentTypes: AppointmentType[]; intakeFields: IntakeField[]; depositRules: DepositRule[]; knowledgeItems: KnowledgeItem[] }) {
  ...
  const [depositEditorMode, setDepositEditorMode] = useState<'create' | 'edit' | null>(null)
  const [knowledgeEditor, setKnowledgeEditor] = useState<KnowledgeEditorState>(null)
  ...
  function clearTypeEditor() {
    setTypeEditor(null)
    setIsCreatingAppointmentType(false)
    setEditingAppointmentTypeId(null)
    setEditingAppointmentTypeIndex(null)
  }
  ...
  function next() { setError(''); clearTypeEditor(); clearQuestionEditor(); clearKnowledgeEditor(); setOnboardingDirection('forward'); setStep((value) => value + 1) }
  function back() { setError(''); clearTypeEditor(); clearQuestionEditor(); clearKnowledgeEditor(); setOnboardingDirection('back'); setStep((value) => value - 1) }
  ...
  function removeDepositRule(id: string) {
    ...
    if (depositEditorId === id) {
      setDepositEditorId(null)
      setDepositEditorMode(null)
    }
  }
  ...
  function startTypeCreate() {
    setEditingAppointmentTypeId(null)
    setIsCreatingAppointmentType(true)
    setTypeEditor({ id: '', name: '', duration_minutes: null, price: null, is_active: true })
  }
  function startTypeEdit(item: EditableAppointmentType, index: number) {
    setIsCreatingAppointmentType(false)
    setEditingAppointmentTypeId(item.id)
    setEditingAppointmentTypeIndex(index)
    setTypeEditor({ ...item })
  }
  function saveTypeEditor() {
    ...
    setTypes((current) => isCreatingAppointmentType
      ? [...current, typeEditor]
      : current.map((item, index) => index === editingAppointmentTypeIndex ? typeEditor : item))
    clearTypeEditor(); setError('')
  }
  function deleteTypeEditor() {
    if (!typeEditor || isCreatingAppointmentType || editingAppointmentTypeIndex === null) return
    typesDirtyRef.current = true
    setTypes((current) => current.filter((_, index) => index !== editingAppointmentTypeIndex))
    clearTypeEditor(); setError('')
  }
  function startQuestionCreate() {
    setEditingQuestionId(null)
    setIsCreatingQuestion(true)
    setQuestionEditor({ id: '', field_key: `custom_${questions.length + 1}`, label: '', is_required: false, is_active: true, sort_order: questions.length })
  }
  function startQuestionEdit(item: IntakeField) {
    setIsCreatingQuestion(false)
    setEditingQuestionId(item.id)
    setQuestionEditor({ ...item })
  }
  function saveQuestionEditor() {
    ...
    setQuestions((current) => isCreatingQuestion
      ? [...current, questionEditor]
      : current.map((item) => item.id === editingQuestionId ? questionEditor : item))
    clearQuestionEditor(); setError('')
  }
  function deleteQuestionEditor() {
    if (!questionEditor || isCreatingQuestion) return
    questionsDirtyRef.current = true
    setQuestions((current) => current.filter((item) => item.field_key !== questionEditor.field_key))
    clearQuestionEditor(); setError('')
  }
  function nextKnowledgeKey() {
    knowledgeKeyRef.current += 1
    return `knowledge-local-${knowledgeKeyRef.current}`
  }
  function startKnowledgeCreate() {
    setKnowledgeEditor({ mode: 'create', item: { id: '', title: '', content: '', localKey: nextKnowledgeKey(), is_active: true, sort_order: knowledge.length } })
  }
  function startKnowledgeEdit(item: EditableKnowledgeItem) {
    setKnowledgeEditor({ mode: 'edit', localKey: item.localKey, item: { ...item } })
  }
  function updateKnowledgeEditor(item: EditableKnowledgeItem) {
    setKnowledgeEditor((current) => current ? { ...current, item } : current)
  }
  function saveKnowledgeEditor() {
    if (!knowledgeEditor?.item.title.trim() || !knowledgeEditor.item.content.trim()) { setError('Completa el tema y la respuesta para continuar.'); return }
    setKnowledge((current) => knowledgeEditor.mode === 'create'
      ? [...current, knowledgeEditor.item]
      : current.map((item) => item.localKey === knowledgeEditor.localKey ? { ...knowledgeEditor.item, localKey: item.localKey } : item))
    clearKnowledgeEditor(); setError('')
  }
  function deleteKnowledgeEditor() {
    if (!knowledgeEditor || knowledgeEditor.mode !== 'edit') return
    if (knowledgeEditor.item.id) setDeletedKnowledgeIds((current) => current.includes(knowledgeEditor.item.id) ? current : [...current, knowledgeEditor.item.id])
    setKnowledge((current) => current.filter((item) => item.localKey !== knowledgeEditor.localKey))
    clearKnowledgeEditor(); setError('')
  }
  function addKnowledgeSuggestion(title: string) { if (knowledge.some((item) => item.title.trim().toLowerCase() === title.toLowerCase())) return; setKnowledgeEditor({ mode: 'create', item: { id: '', title, content: '', localKey: nextKnowledgeKey(), is_active: true, sort_order: knowledge.length } }) }
  function persistAll() {
    ...
    startTransition(async () => {
      ...
      for (const id of deletedKnowledgeIds) {
        const result = await deleteKnowledgeItem(makeData(doctorId, { id }))
        if (result.error) { setCompletionStatus('error'); return }
      }
...
function TypeEditor({ item, onChange, onSave, onDelete, onCancel }: { item: EditableAppointmentType; onChange: (value: EditableAppointmentType) => void; onSave: () => void; onDelete?: () => void; onCancel: () => void }) { return <section className="assistant-conditional-enter space-y-4 rounded-lg border border-zinc-300 bg-white p-4" aria-label="Editor de tipo de cita"><div className="grid gap-3 sm:grid-cols-[1fr_180px_140px]"><Field label="Nombre de la cita"><input className={inputClass} autoFocus value={item.name} onChange={(event) => onChange({ ...item, name: event.target.value })} placeholder="Primera consulta" /></Field><Field label="Duración"><div className="flex items-center gap-2"><input className={inputClass} type="number" min="1" value={item.duration_minutes ?? ''} onChange={(event) => onChange({ ...item, duration_minutes: event.target.value ? Number(event.target.value) : null })} placeholder="Ej. 60" /><span className="shrink-0 text-sm text-zinc-500">minutos</span></div></Field><Field label="Costo opcional"><div className="flex items-center"><span className="rounded-l-lg border border-r-0 border-zinc-300 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-500">$</span><input className={`${inputClass} rounded-l-none`} type="number" min="0" step="0.01" value={item.price ?? ''} onChange={(event) => onChange({ ...item, price: event.target.value ? Number(event.target.value) : null })} placeholder="Opcional" /></div></Field></div><div className="flex flex-wrap items-center justify-between gap-3 border-t border-zinc-100 pt-3"><div>{onDelete ? <button type="button" onClick={onDelete} className="text-sm font-medium text-red-700 transition hover:text-red-800">Eliminar</button> : null}</div><div className="flex items-center justify-end gap-3"><button type="button" onClick={onCancel} className={secondaryButtonClass}>Cancelar</button><Button onClick={onSave}>{item.id ? 'Guardar' : 'Agregar'}</Button></div></div></section> }
function QuestionEditor({ item, onChange, onSave, onDelete, onCancel }: { item: IntakeField; onChange: (value: IntakeField) => void; onSave: () => void; onDelete?: () => void; onCancel: () => void }) { return <section className="assistant-conditional-enter space-y-3 rounded-lg border border-zinc-300 bg-white p-4" aria-label="Editor de pregunta personalizada"><Field label="Pregunta"><input className={inputClass} autoFocus value={item.label} onChange={(event) => onChange({ ...item, label: event.target.value })} placeholder="Otra pregunta" /></Field><label className="flex items-center gap-2 text-sm text-zinc-700"><input className="h-4 w-4 accent-zinc-900" type="checkbox" checked={item.is_required} onChange={(event) => onChange({ ...item, is_required: event.target.checked })} />Necesaria</label><div className="flex items-center justify-between gap-3 border-t border-zinc-100 pt-3"><div>{onDelete ? <button type="button" onClick={onDelete} className="text-sm font-medium text-red-700 transition hover:text-red-800">Eliminar</button> : null}</div><div className="flex justify-end gap-3"><button type="button" onClick={onCancel} className={secondaryButtonClass}>Cancelar</button><Button onClick={onSave}>{item.id ? 'Guardar' : 'Agregar'}</Button></div></div></section> }
function KnowledgeReadRow({ item, onEdit }: { item: EditableKnowledgeItem; onEdit: () => void }) { return <CollectionReadRow title={item.title || 'Información sin título'} meta={item.content || 'Sin respuesta'} onEdit={onEdit} /> }
function KnowledgeEditor({ item, onChange, onSave, onDelete, onCancel }: { item: EditableKnowledgeItem; onChange: (value: EditableKnowledgeItem) => void; onSave: () => void; onDelete?: () => void; onCancel: () => void }) { return <section className="assistant-conditional-enter space-y-4 rounded-lg border border-zinc-300 bg-white p-4" aria-label="Editor de información"><Field label="Tema o pregunta"><input className={inputClass} autoFocus value={item.title} onChange={(event) => onChange({ ...item, title: event.target.value })} placeholder="¿Hay estacionamiento?" /></Field><Field label="Respuesta"><textarea className={inputClass} rows={3} value={item.content} onChange={(event) => onChange({ ...item, content: event.target.value })} placeholder="Escribe una respuesta para tus pacientes" /></Field><div className="flex items-center justify-between gap-3 border-t border-zinc-100 pt-3"><div>{onDelete ? <button type="button" onClick={onDelete} className="text-sm font-medium text-red-700 transition hover:text-red-800">Eliminar</button> : null}</div><div className="flex justify-end gap-3"><button type="button" onClick={onCancel} className={secondaryButtonClass}>Cancelar</button><Button onClick={onSave}>{onDelete ? 'Guardar' : 'Agregar'}</Button></div></div></section> }
function DepositRuleReadRow({ rule, appointmentTypes, onEdit, onRemove }: { rule: DepositRuleDraft; appointmentTypes: Array<Pick<AppointmentType, 'id' | 'name'>>; onEdit: () => void; onRemove: () => void }) { const appointmentTypeName = rule.appointment_type_id ? appointmentTypes.find((item) => item.id === rule.appointment_type_id)?.name : null; const subject = rule.scope === 'ALL' ? 'Todas las citas' : rule.scope === 'APPOINTMENT_TYPE' ? appointmentTypeName || 'El tipo de cita que corresponda' : rule.condition_value || 'La condición que corresponda'; const amount = rule.deposit_type === 'PERCENTAGE' ? 'Porcentaje pendiente' : rule.deposit_value ? `$${rule.deposit_value}` : 'Cantidad pendiente'; return <div className="assistant-collection-row flex items-center justify-between gap-4 rounded-lg border border-zinc-200 bg-white px-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-zinc-900">{rule.name || subject}</p><p className="mt-1 text-xs text-zinc-500">{rule.name ? `${subject} · ` : ''}{amount} · {rule.is_active ? 'Activa' : 'Inactiva'}</p></div><div className="flex shrink-0 items-center gap-2"><button type="button" onClick={onEdit} className={secondaryButtonClass}>Editar</button><button type="button" onClick={onRemove} className="text-sm font-medium text-red-700 transition hover:text-red-800">Eliminar</button></div></div> }

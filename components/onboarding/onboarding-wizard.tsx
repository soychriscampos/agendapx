...
function DepositRuleEditor({ rule, enabled, index, count, appointmentTypes, intakeFields, onChange, onMove, onRemove }: { rule: DepositRuleDraft; enabled: boolean; index: number; count: number; appointmentTypes: Array<Pick<AppointmentType, 'id' | 'name'>>; intakeFields: Array<Pick<IntakeField, 'id' | 'label'>>; onChange: (changes: Partial<DepositRuleDraft>) => void; onMove: (direction: -1 | 1) => void; onRemove: () => void }) {
  return <section className="space-y-4 rounded-lg border border-zinc-200 p-4" aria-label={`Regla de anticipo ${index + 1}`}>
    ...
    <div><label className="block text-xs font-medium text-zinc-600">Cantidad de anticipo<div className="mt-1 flex items-center"><span className="rounded-l-lg border border-r-0 border-zinc-300 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-500">$</span><input className={`${inputClass} rounded-l-none`} type="number" min="0.01" step="0.01" value={rule.deposit_type === 'PERCENTAGE' ? '' : rule.deposit_value ?? ''} onChange={(event) => onChange({ deposit_type: 'FIXED', deposit_value: event.target.value ? Number(event.target.value) : null })} placeholder="Ej. 500" /></div></label>{rule.legacyPercentage && (rule.deposit_type === 'PERCENTAGE' || !(rule.deposit_value !== null && Number.isFinite(rule.deposit_value) && rule.deposit_value > 0)) ? <p className="mt-2 text-xs leading-5 text-amber-700">Esta regla usaba porcentaje. Ingresa una cantidad de anticipo para actualizarla.</p> : null}</div>
